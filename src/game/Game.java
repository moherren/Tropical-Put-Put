	...
	public void loadCourse(GolfCourse gc){
		...
		if((difficulty>0&&(screen==SC_GOLF_GAME||backScreen==SC_GOLF_GAME))){
			if(difficulty!=1)
				...
				course.setTilt(new Vector2D(Math.random()*2-1,Math.random()*2-1).normalize(),Math.pow(Math.random(),0.25)*course.maxTilt);
				course.tiltVelocity=new Vector2D(Math.random()*2-1,Math.random()*2-1).normalize().mult(0.75);
			}
		}
